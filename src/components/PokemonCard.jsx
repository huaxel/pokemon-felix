...
import {
  mysteryBoxIcon as favIcon,
  greatballIcon as squadIcon,
} from '../features/world/worldAssets';
...
const PokemonCardComponent = ({
  ...
  const displayName =
    pokemon.speciesData?.names?.find(n => n.language.name === 'en')?.name || pokemon.name;
    ...
    >
      <div className="card-header">
        ...
        >
          <img
            src={favIcon}
            alt={isOwned ? 'Verwijder favoriet' : 'Markeer favoriet'}
            className="icon"
          />
        </button>
        {onToggleSquad && (
          ...
          >
            <img
              src={squadIcon}
              alt={isInSquad ? 'Verwijder uit team' : 'Voeg toe aan team'}
              className="icon"
            />
          </button>
        )}
