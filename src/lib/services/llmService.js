 ...
 * Should be called periodically to condense memory.
 */
export async function summarizeConversation(history, currentSummary) {
  if (!LLM_API_KEY) return currentSummary;

  const prompt = `
    Summarize the following Pokémon trainer interaction history into a single concise sentence.
    Integrate it with the existing history: "${currentSummary}"
