...
export const handleCardActions = (state, action) => {
  switch (action.type) {
    case BATTLE_ACTIONS.SET_HAND:
      return { ...state, cards: { ...state.cards, hand: action.hand } };
    case BATTLE_ACTIONS.SET_DECK:
      return { ...state, cards: { ...state.cards, deck: action.deck } };
    case BATTLE_ACTIONS.SELECT_CARD:
      if (state.cards.selectedIndices.includes(action.index)) return state;
      return {
        ...state,
        cards: { ...state.cards, selectedIndices: [...state.cards.selectedIndices, action.index] },
      };
...
export const handleUIActions = (state, action) => {
  switch (action.type) {
    case BATTLE_ACTIONS.SET_ATTACKING_FIGHTER:
      return { ...state, ui: { ...state.ui, attackingFighter: action.fighter } };
    case BATTLE_ACTIONS.SET_DAMAGED_FIGHTER:
      return { ...state, ui: { ...state.ui, damagedFighter: action.fighter } };
    case BATTLE_ACTIONS.SET_EFFECTIVENESS_MSG:
      return { ...state, ui: { ...state.ui, effectivenessMsg: action.message } };
    case BATTLE_ACTIONS.SET_COMBO_MSG:
      return { ...state, ui: { ...state.ui, comboMsg: action.message } };
    ...
    case BATTLE_ACTIONS.TOGGLE_ITEMS:
      return { ...state, ui: { ...state.ui, showItems: !state.ui.showItems } };
    default:
      return state;
  }
};
