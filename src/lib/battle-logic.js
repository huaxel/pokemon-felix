...
import { TYPE_CHART, TYPE_COLORS, MOVE_POOL, DEFAULT_MOVES } from './battleConfig';
...
export const getEffectiveness = (attackerType, defenderType) => {
  ...
  return attackerChart[defenderType.toLowerCase()] !== undefined
    ? attackerChart[defenderType.toLowerCase()]
    : 1;
};
...
export const calculateDamage = (attacker, defender, move) => {
  ...
  if (effectiveness > 1) {
    ...
    finalDamage = Math.max(1, finalDamage - 1); // Not very effective = -1 Dmg (min 1)
...
const getFusionMove = (m1, m2) => {
  ...
  if ((t1 === 'fire' && t2 === 'water') || (t1 === 'water' && t2 === 'fire')) {
    return {
      ...
      type: 'water',
      power: 100,
      description: 'Kokende stoom verbrandt de tegenstander.',
    };
  }
  if ((t1 === 'electric' && t2 === 'water') || (t1 === 'water' && t2 === 'electric')) {
    return {
      ...
      type: 'electric',
      power: 110,
      description: 'Geëlektrificeerd water geleidt perfect.',
    };
  }
  return null;
};
...
const getBaseCombo = (m1, m2, isSame) => ({
  ...
  power: isSame
    ? Math.floor(((m1.power || 40) + (m2.power || 40)) * 1.2)
    : Math.floor((m1.power || 40) + (m2.power || 40)),
