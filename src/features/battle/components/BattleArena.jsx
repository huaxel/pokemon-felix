...
export function BattleArena({ initialFighter1, initialFighter2, onBattleEnd }) {
    ...
    const executeMove = useCallback(async (attacker, defender, move, isPlayer) => {
        ...
        const { damage, recoilDamage } = result;
        const color = getTypeColor(attacker.types?.[0]?.type?.name);

        if (isPlayer) {
            const nextHP = Math.max(0, f2HP - damage);
            setF2HP(nextHP);
            addLog(`¡${attacker.name} usa ${move.name}! -${damage} HP${result.message}`, color);

            if (f2Weakened) setF2Weakened(false); // Reset after being hit

            if (recoilDamage > 0) {
                const nextF1HP = Math.max(0, f1HP - recoilDamage);
                setF1HP(nextF1HP);
                addLog(`¡${attacker.name} recibe -${recoilDamage} HP por retroceso!`, color);
                setF1Weakened(true); // Weak after recoil
                if (nextF1HP === 0) { endBattle(2); return; }
            }
            ...
            setF1HP(nextHP);
            addLog(`¡Enemigo ${attacker.name} usa ${move.name}! -${damage} HP${result.message}`, color);

            if (f1Weakened) setF1Weakened(false); // Reset after being hit

            if (recoilDamage > 0) {
                const nextF2HP = Math.max(0, f2HP - recoilDamage);
                setF2HP(nextF2HP);
                addLog(`¡Enemigo recibe -${recoilDamage} HP por retroceso!`, color);
                setF2Weakened(true); // Weak after recoil
                if (nextF2HP === 0) { endBattle(1); return; }
            }
    ...
    }, [f1HP, f2HP, f1Weakened, f2Weakened, endBattle]);
