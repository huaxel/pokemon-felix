...
export function PorygonControls({ program, limit, isRunning, onAdd, onRemove, onRun, onReset }) {
  return (
    <div className="controls-area">
      <div className="program-display">
        <h3>
          Programa ({program.length}/{limit})
        </h3>
      ...
      <div className="command-palette">
        <button
          className="cmd-btn"
          onClick={() => onAdd('UP')}
          disabled={isRunning || program.length >= limit}
        >
          <ArrowUp /> Avanzar
        </button>
        <button
