    ...
    const runProgram = async () => {
        ...
        for (const cmd of program) {
            ...
            if (cmd === 'LEFT') currentPos.dir = (currentPos.dir + 3) % 4;
                ...
                const nx = currentPos.x + dx, ny = currentPos.y + dy;

                if (nx >= 0 && nx < 5 && ny >= 0 && ny < 5 && (level.grid[ny][nx] >= 1)) {
                    currentPos.x = nx; currentPos.y = ny;
                } else {
    ...
    return (
        <div className="porygon-lab">
            ...
            <div className="lab-content">
                ...
                <PorygonControls
                    program={program} limit={level?.limit || 5} isRunning={isRunning}
                    onAdd={addToProgram} onRemove={(i) => !isRunning && setProgram(program.filter((_, idx) => idx !== i))}
                    onRun={runProgram} onReset={() => { setProgram([]); setPorygonState({ ...level.start }); setMessage(null); }}
                />
            </div>
        </div>
    );
