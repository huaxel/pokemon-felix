    ...
    return (
        <>
                ...
                {Object.values(tiers).filter(t => t.type === category).map(tier => (
                    ...
                    >
                        {typeof tier.image === 'string' && tier.image.length < 4 ? (
                            <div className="item-emoji" style={{ fontSize: '2rem' }}>{tier.image}</div>
                        ) : (
                        ...
                        <div className="ball-info">
                            ...
                            <span className="ball-cost">
                                <img src={bagIcon} alt="coins" className="coin-icon" style={{ width: '12px', height: '12px' }} />
                                {tier.cost}
                            </span>
                        </div>
                    </div>
            ...
            <div className="summon-container">
                ...
                />
                <p style={{ fontFamily: '"Press Start 2P", cursive', fontSize: '0.7rem', color: '#fbbf24', textShadow: '1px 1px 0 #000' }}>Tik op de {currentTier.name} om te kopen</p>
                <button className="summon-btn btn-kenney warning" onClick={onSummon}>
                    Kopen (<img src={bagIcon} alt="coins" className="coin-icon-inline" style={{ width: '16px', verticalAlign: 'middle' }} /> {currentTier.cost})
                </button>
            </div>
        </>
    );
