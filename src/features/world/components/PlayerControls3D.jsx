...
export function PlayerControls3D({ onLock, onUnlock, mapGrid = [], initialPos, onPositionChange }) {
  ...
  const lastGridPos = useRef({
    x: Math.round(initialPos?.x || 0),
    y: Math.round(initialPos?.y || 0),
  });
  ...
  return <PointerLockControls ref={controlsRef} onLock={onLock} onUnlock={onUnlock} />;
}
