...
export function PotionLabPage() {
  ...
  useEffect(() => {
    setCurrentValue(selectedIngredients.reduce((s, i) => s + i.value, 0));
  }, [selectedIngredients]);
  useEffect(() => {
  ...
  const brewPotion = () => {
    if (currentValue === targetValue) {
      ...
      addCoins(reward);
      setCompletedPotions(p => p + 1);
      setStreak(s => s + 1);
      showSuccess(`¡Perfecto! +${reward} 💰`, 'success');
      setTimeout(generateNewChallenge, 2000);
    } else {
      setStreak(0);
      showError(`Error: tienes ${currentValue}, necesitas ${targetValue}`, 'error');
    }
  };
    ...
    <div className="potion-lab-page">
      ...
      <div className="difficulty-selector">
        {Object.entries(DIFFICULTIES).map(([k, v]) => (
          <button
            key={k}
            className={difficulty === k ? 'active' : ''}
            onClick={() => setDifficulty(k)}
            style={{ background: difficulty === k ? `${diffColor()}33` : 'rgba(255,255,255,0.1)' }}
          >
            {v.name}
          </button>
        ))}
      ...
      <div className="lab-intro">
        ...
        <div className="lab-dialog">
          <p>
            ¡Bienvenido al Laboratorio! Tu reto hoy es crear una poción con valor{' '}
      ...
      <PotionBrewingStation
        ...
        onRemoveLast={() => setSelectedIngredients(s => s.slice(0, -1))}
      ...
      <PotionIngredientsPanel
        ...
        onAdd={i => setSelectedIngredients(s => [...s, { ...i, id: Math.random() }])}
      />

      <div className="brew-section">
        <button className="btn-adventure primary brew-button" onClick={brewPotion}>
          ¡Crear Poción!
        </button>
