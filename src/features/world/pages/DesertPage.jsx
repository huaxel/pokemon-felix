...
export function DesertPage() {
    ...
    const getTileContent = (x, y) => {
        if (x === playerPos.x && y === playerPos.y) {
            return <img src={playerTile} alt="Player" style={{ width: '80%', height: '80%', imageRendering: 'pixelated' }} />;
        }
        if (isOasis(x, y)) {
            if (discoveredOases.some(o => o.x === x && o.y === y)) {
                return <img src={waterImage} alt="Oasis" style={{ width: '100%', height: '100%', imageRendering: 'pixelated', opacity: 0.8 }} />;
            }
        ...
        if ((x * 7 + y * 13) % 10 > 7) {
            return <img src={desertCactusTile} alt="Cactus" style={{ width: '80%', height: '80%', imageRendering: 'pixelated' }} />;
        }
        return null;
    };
    ...
    return (
        <div className={`desert-page ${sandstorm ? 'sandstorm' : ''}`} style={{
            ...
            {showFact && (
                <div className="geography-fact game-panel" style={{ border: '4px solid #92400e', backgroundColor: '#fef3c7', padding: '1rem', marginBottom: '1rem', position: 'relative' }}>
                    {GEOGRAPHY_FACTS[Math.floor(Math.random() * GEOGRAPHY_FACTS.length)]}
                    <button className="fact-close" onClick={() => setShowFact(false)} style={{ position: 'absolute', top: '5px', right: '5px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 'bold' }}>✕</button>
                </div>
            )}

            <div className="desert-stats game-panel-dark" style={{ border: '4px solid #4b5563', backgroundColor: '#1f2937', padding: '0.5rem', color: '#fff', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}>
                <span>Positie: ({playerPos.x}, {playerPos.y})</span>
                <span>Oases ontdekt: {discoveredOases.length}/3</span>
            </div>

            <div className="desert-grid" style={{ display: 'grid', gridTemplateColumns: `repeat(${DESERT_SIZE}, 1fr)`, gap: '2px', backgroundColor: '#78350f', padding: '4px', borderRadius: '4px' }}>
            ...
            <div className="desert-controls" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', marginTop: '1rem' }}>
                <button className="btn-kenney" onClick={() => handleMove(0, -1)}>⬆️</button>
                <div className="control-row" style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="btn-kenney" onClick={() => handleMove(-1, 0)}>⬅️</button>
                    <button className="btn-kenney" onClick={() => handleMove(1, 0)}>➡️</button>
                </div>
                <button className="btn-kenney" onClick={() => handleMove(0, 1)}>⬇️</button>
            </div>
            ...
            {encounter && (
                <div className="encounter-modal" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
                    <div className="encounter-content game-panel" style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '1rem', border: '4px solid #4b5563', maxWidth: '90%' }}>
                        <h2 style={{ fontFamily: '"Press Start 2P", cursive', fontSize: '1rem', textAlign: 'center', marginBottom: '1rem' }}>Wilde Pokémon!</h2>
