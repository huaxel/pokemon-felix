...
const TREASURE_HUNTS = [
    { id: 1, difficulty: 'Makkelijk', target: { x: 5, y: 5 }, clue: "De schat ligt in het centrum van de stad, waar Professor Oak vaak staat!", reward: 300, pokemonReward: 'pikachu' },
    { id: 2, difficulty: 'Makkelijk', target: { x: 8, y: 8 }, clue: "Kijk in de zuidoostelijke hoek, vlakbij de Gacha machine!", reward: 350, pokemonReward: 'eevee' },
    { id: 3, difficulty: 'Gemiddeld', target: { x: 3, y: 7 }, clue: "De schat is ten westen van het pad, vlakbij het Gym gebouw!", reward: 500, pokemonReward: 'bulbasaur' },
    { id: 4, difficulty: 'Gemiddeld', target: { x: 9, y: 4 }, clue: "Zoek in het oostelijke watergebied, bij het Fonteinplein!", reward: 550, pokemonReward: 'squirtle' },
    { id: 5, difficulty: 'Moeilijk', target: { x: 1, y: 1 }, clue: "In de noordwestelijke hoek van de wereld, waar weinig reizigers komen...", reward: 800, pokemonReward: 'charmander' },
    { id: 6, difficulty: 'Moeilijk', target: { x: 6, y: 9 }, clue: "Aan de zuidelijke grens, vlakbij de Evolutiehal!", reward: 850, pokemonReward: 'dratini' },
    { id: 7, difficulty: 'Expert', target: { x: 9, y: 7 }, clue: "Hoog in de bergen, waar de lucht ijl is...", reward: 1200, pokemonReward: 'larvitar' },
    { id: 8, difficulty: 'Expert', target: { x: 0, y: 0 }, clue: "Het meest noordwestelijke punt van de hele wereld!", reward: 1500, pokemonReward: 'beldum' }
];
...
export function TreasureHuntPage() {
    ...
    return (
        <div className="treasure-hunt-page-wrapper" style={{ backgroundImage: `url(${grassTile})`, imageRendering: 'pixelated' }}>
            <WorldPageHeader title="Schattenjacht" icon="🗺️" />
            {stage === 'tutorial' && <TreasureTutorial huntsCompleted={huntsCompleted} totalHunts={TREASURE_HUNTS.length} onStart={startNewHunt} onBack={() => navigate('/adventure')} />}
            {stage === 'hunting' && <TreasureHuntingView onBack={() => setStage('tutorial')} currentHunt={currentHunt} attempts={attempts} distance={distance} message={message} playerGuess={playerGuess} setPlayerGuess={setPlayerGuess} onGuess={handleGuess} />}
            {stage === 'success' && <TreasureSuccessView currentHunt={currentHunt} attempts={attempts} rewardPokemon={rewardPokemon} onNext={startNewHunt} />}
            {stage === 'failed' && <TreasureFailedView currentHunt={currentHunt} onRestart={() => { setAttempts(0); setStage('hunting'); setMessage(''); }} onBack={() => setStage('tutorial')} />}
        </div>
    );
}
