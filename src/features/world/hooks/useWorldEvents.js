...
const handleGrassEncounter = (getEncounterMultiplier, navigateWithMessage, showMessage, addItem, addCoins) => {
    ...
    if (r < 0.05) {
        navigateWithMessage("¡✨ LEYENDA ✨!", '/single-battle', '#f59e0b', 2000, { isWild: true, isLegendary: true });
    } else if (r < 0.6) {
...
export function useWorldEvents({
    playerPos, questState, setQuestState, treeCount, addCoins, addItem,
    treasures, setTreasures, pokeballs, setPokeballs, targetPos, showMessage, navigateWithMessage,
    setShowInterior, getEncounterMultiplier, generateRandomTarget
}) {
    return useCallback((tileType) => {
        if (getNPCResponse(tileType, { playerPos, questState, treeCount, addCoins, addItem, setQuestState, showMessage })) return;
        ...
        if (tileType === TILE_TYPES.CENTER) { return navigateWithMessage("Centro Pokémon", '/center', '#ef4444'); }
        
        if (tileType === TILE_TYPES.GRASS) {
            handleGrassEncounter(getEncounterMultiplier, navigateWithMessage, showMessage, addItem, addCoins);
            return;
        }
        ...
        if (handleGPSTarget(playerPos, targetPos, showMessage, addCoins, generateRandomTarget)) return;

    }, [playerPos, questState, treeCount, treasures, pokeballs, targetPos, addCoins, addItem, setQuestState, showMessage, navigateWithMessage, setShowInterior, getEncounterMultiplier, generateRandomTarget, setTreasures, setPokeballs]);
}
