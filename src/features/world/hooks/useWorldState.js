...
export function useWorldState() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (
        treasures.length < TREASURE_CONFIG.MAX_TREASURES &&
        Math.random() < TREASURE_CONFIG.SPAWN_CHANCE
      ) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (
        pokeballs.length < POKEBALL_CONFIG.MAX_POKEBALLS &&
        Math.random() < POKEBALL_CONFIG.SPAWN_CHANCE
      ) {
        const newX = Math.floor(Math.random() * 10);
        const newY = Math.floor(Math.random() * 10);
