                            ...
                            return (
                                <div key={matchIndex} className={`bracket-match ${isCurrent ? 'active-match' : ''}`}>
                                    <div className={`match-participant ${match.winner === match.p1 ? 'winner' : ''}`}>
                                        {match.p1 && <img src={match.p1.sprites.front_default} alt={match.p1.name} className="bracket-icon" />}
                                        <span>{match.p1 ? match.p1.name : '???'}</span>
                                    </div>
                                    <div className={`match-participant ${match.winner === match.p2 ? 'winner' : ''}`}>
                                        {match.p2 && <img src={match.p2.sprites.front_default} alt={match.p2.name} className="bracket-icon" />}
